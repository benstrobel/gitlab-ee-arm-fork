    ...
    class Git
      class << self
        def branch_name
          ...
          head_reference = OmnibusGitlab::Util.shellout_stdout('git rev-parse --abbrev-ref HEAD')
        ...
        def tag_name
          Build::Info::CI.tag_name || OmnibusGitlab::Util.shellout_stdout('git describe --tags --exact-match')
        rescue OmnibusGitlab::Util::ShellOutExecutionError => e
        ...
        def commit_sha
          commit_sha_raw = OmnibusGitlab::Util.get_env('CI_COMMIT_SHA') || OmnibusGitlab::Util.shellout_stdout('git rev-parse HEAD')
        ...
        def latest_tag
          unless (fact_from_file = OmnibusGitlab::Util.fetch_fact_from_file(__method__)).nil?
        ...
        def sorted_tags_for_edition
          OmnibusGitlab::Util.shellout_stdout("git -c versionsort.prereleaseSuffix=rc tag -l '#{tag_match_pattern}' --sort=-v:refname")&.split("\n")
        end
