...
module Build
  class Info
    class CI
      class << self
        def branch_name
          OmnibusGitlab::Util.get_env('CI_COMMIT_BRANCH')
        end
        ...
        def project_id
          OmnibusGitlab::Util.get_env('CI_PROJECT_ID')
        end

        def pipeline_id
          OmnibusGitlab::Util.get_env('CI_PIPELINE_ID')
        end

        def job_id
          OmnibusGitlab::Util.get_env('CI_JOB_ID')
        end

        def job_token
          OmnibusGitlab::Util.get_env('CI_JOB_TOKEN')
        end

        def api_v4_url
          OmnibusGitlab::Util.get_env('CI_API_V4_URL')
        end

        def commit_ref_slug
          OmnibusGitlab::Util.get_env('CI_COMMIT_REF_SLUG')
        end

        def artifact_url(job_name, file_path)
          client = OmnibusGitlab::APIClient.new
