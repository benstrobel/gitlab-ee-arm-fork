      ...
      class GitLabRails
        class << self
          ...
          def version
            ...
            if OmnibusGitlab::Util.get_env('GITLAB_VERSION').nil? || OmnibusGitlab::Util.get_env('GITLAB_VERSION').empty?
              File.read('VERSION').strip
            else
              OmnibusGitlab::Util.get_env('GITLAB_VERSION')
            end
          end
          ...
          def ref(prepend_version: true)
            ...
            #    rails. This will be used by `OmnibusGitlab::Version` class and will be
            ...
            #    can directly use it.
            OmnibusGitlab::Version.new(name).print(prepend_version)
          end

          def project_path
            if OmnibusGitlab::Util.get_env('CI_SERVER_HOST') == 'dev.gitlab.org'
              Build::Info::Package.name == "gitlab-ee" ? 'gitlab/gitlab-ee' : 'gitlab/gitlabhq'
            else
              namespace = OmnibusGitlab::Version.security_channel? ? "gitlab-org/security" : "gitlab-org"
          ...
          def repo
            OmnibusGitlab::Version.new(name).remote
          end
        end
      end
    end
