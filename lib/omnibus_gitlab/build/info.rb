  ...
  class Info
    ...
    class << self
      def gcp_release_bucket
        ...
        gcp_pkgs_release_bucket = OmnibusGitlab::Util.get_env('GITLAB_COM_PKGS_RELEASE_BUCKET') || 'gitlab-com-pkgs-release'
        gcp_pkgs_builds_bucket = OmnibusGitlab::Util.get_env('GITLAB_COM_PKGS_BUILDS_BUCKET') || 'gitlab-com-pkgs-builds'
        Check.on_tag? ? gcp_pkgs_release_bucket : gcp_pkgs_builds_bucket
      end

      def gcp_release_bucket_sa_file
        OmnibusGitlab::Util.get_env('GITLAB_COM_PKGS_SA_FILE')
      end

      def log_level
        if OmnibusGitlab::Util.get_env('BUILD_LOG_LEVEL') && !OmnibusGitlab::Util.get_env('BUILD_LOG_LEVEL').empty?
          OmnibusGitlab::Util.get_env('BUILD_LOG_LEVEL')
        else
          'info'
        end
      end
      ...
      def deploy_env
        ...
        env = OmnibusGitlab::Util.get_env(key)
