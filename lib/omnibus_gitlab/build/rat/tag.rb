      ...
      def self.get_params(image: nil)
        {
          ...
          'variables[QA_IMAGE]' => OmnibusGitlab::Util.get_env('QA_IMAGE') || image || "dev.gitlab.org:5005/gitlab/gitlab-ee/gitlab-ee-qa:#{version.partition(/\.\d+$/).first}"
        }
      end

      def self.version
        OmnibusGitlab::Util.get_env('CI_COMMIT_TAG').tr('+', '-')
      end
