  ...
  class RAT
    class NightlyPipeline
      ...
      def self.get_params(image: nil)
        {
          'ref' => 'master',
          'token' => OmnibusGitlab::Util.get_env('RAT_TRIGGER_TOKEN'),
          'variables[REFERENCE_ARCHITECTURE]' => OmnibusGitlab::Util.get_env('RAT_REFERENCE_ARCHITECTURE') || 'omnibus-gitlab-mrs',
          'variables[NIGHTLY]' => "true",
          'variables[PACKAGE_VERSION]' => Build::Info::Package.name_version,
          'variables[QA_IMAGE]' => OmnibusGitlab::Util.get_env('QA_IMAGE') || image || 'gitlab/gitlab-ee-qa:nightly'
        }
      end
      ...
      def self.get_access_token
        ...
        OmnibusGitlab::Util.get_env('RAT_PROJECT_ACCESS_TOKEN')
      end
    end
  end
end
