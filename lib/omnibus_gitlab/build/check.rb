...
module Build
  class Check
    ...
    class << self
      def is_ee?
        OmnibusGitlab::Util.get_env('ee') == 'true' || \
          OmnibusGitlab::Util.get_env('GITLAB_VERSION')&.end_with?('-ee') || \
          File.read('VERSION').strip.end_with?('-ee') || \
          is_auto_deploy?
      end

      def is_jh?
        OmnibusGitlab::Util.get_env('jh') == 'true' || \
          OmnibusGitlab::Util.get_env('GITLAB_VERSION')&.end_with?('-jh') || \
      ...
      def use_system_ssl?
        ...
        # gate on `fips?` alone.
        OmnibusGitlab::Util.get_env('USE_SYSTEM_SSL') == 'true' || fips?
      end
      ...
      def is_auto_deploy_branch?
        OmnibusGitlab::Util.get_env('CI_COMMIT_REF_NAME')&.include?('-auto-deploy-')
      end
      ...
      def ci_commit_tag?
        OmnibusGitlab::Util.get_env('CI_COMMIT_TAG')
      end
      ...
      def is_nightly?
        OmnibusGitlab::Util.get_env('NIGHTLY') == 'true'
      end
