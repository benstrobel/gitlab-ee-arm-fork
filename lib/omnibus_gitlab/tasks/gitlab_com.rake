...
namespace :gitlab_com do
  ...
  task :deployer do
    abort "This task requires DEPLOYER_TRIGGER_TOKEN to be set" unless OmnibusGitlab::Util.get_env('DEPLOYER_TRIGGER_TOKEN')
    ...
    trigger_token = OmnibusGitlab::Util.get_env('DEPLOYER_TRIGGER_TOKEN')
    trigger_ref = Build::Check.is_auto_deploy? && Build::Check.ci_commit_tag? ? OmnibusGitlab::Util.get_env('CI_COMMIT_TAG') : :master
