...
namespace :build do
  ...
  task project: ["cache:purge", "check:no_changes"] do
    OmnibusGitlab::Util.section('build:project') do
  ...
  namespace :package do
    ...
    task :generate_checksums do
      OmnibusGitlab::Util.section('build:package:generate_checksums') do
    ...
    task :generate_sizefile do
      OmnibusGitlab::Util.section('build:package:generate_sizefile') do
        files = Dir.glob('pkg/**/*.{deb,rpm}').select { |f| File.file? f }
        if files.empty?
    ...
    task :sync do
      OmnibusGitlab::Util.section('build:package:sync', collapsed: Build::Check.on_tag?) do
  ...
  task :version do
    ...
    puts OmnibusGitlab::Version.new('gitlab-rails').print
  end
  ...
  task :component_shas do
    ...
    OmnibusGitlab::Util.section('build:component_shas') do
