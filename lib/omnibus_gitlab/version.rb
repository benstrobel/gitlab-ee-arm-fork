...
module OmnibusGitlab
  class Version
    ...
    def self.sources_channel
      return SECURITY_SOURCE if OmnibusGitlab::Util.get_env("SECURITY_SOURCES").to_s == "true"
    ...
    def self.fallback_sources_channel
      OmnibusGitlab::Util.get_env("ALTERNATIVE_SOURCES").to_s == "false" ? DEFAULT_SOURCE : ALTERNATIVE_SOURCE
    end
    ...
    def read_version_from_env
      OmnibusGitlab::Util.get_env(COMPONENTS_ENV_VARS[@software]) if COMPONENTS_ENV_VARS.include?(@software)
    end
    ...
    def read_remote_from_env
      remote = case @software
               when "gitlab-rails", "gitlab-rails-ee"
                 OmnibusGitlab::Util.get_env("GITLAB_ALTERNATIVE_REPO")
               when "gitlab-shell"
                 OmnibusGitlab::Util.get_env("GITLAB_SHELL_ALTERNATIVE_REPO")
               when "gitlab-pages"
                 OmnibusGitlab::Util.get_env("GITLAB_PAGES_ALTERNATIVE_REPO")
               when "gitaly"
                 OmnibusGitlab::Util.get_env("GITALY_SERVER_ALTERNATIVE_REPO")
               when "gitlab-elasticsearch-indexer"
                 OmnibusGitlab::Util.get_env("GITLAB_ELASTICSEARCH_INDEXER_ALTERNATIVE_REPO")
               when "gitlab-kas"
                 OmnibusGitlab::Util.get_env("GITLAB_KAS_ALTERNATIVE_REPO")
               end

      if remote && OmnibusGitlab::Util.get_env("ALTERNATIVE_PRIVATE_TOKEN")
        attach_remote_credential(remote, OmnibusGitlab::Util.get_env("ALTERNATIVE_PRIVATE_TOKEN"))
      else
        remote
      end
    end
    ...
    def read_remote_from_file(channel = nil)
      ...
      channel ||= ::OmnibusGitlab::Version.sources_channel

      return "" unless sources

      if channel == SECURITY_SOURCE
        attach_remote_credential(sources[channel], OmnibusGitlab::Util.get_env("CI_JOB_TOKEN")) || sources[::OmnibusGitlab::Version.fallback_sources_channel]
      else
        sources[channel]
      end
    end
