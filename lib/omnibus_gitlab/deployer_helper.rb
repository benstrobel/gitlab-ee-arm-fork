...
class DeployerHelper
  ...
  def trigger_host
    @trigger_host ||= OmnibusGitlab::Util.get_env('DEPLOYER_TRIGGER_HOST') || 'ops.gitlab.net'
  end

  def trigger_project
    # Project ID 135 is the project ID of deployer on ops.gitlab.net
    @trigger_project ||= OmnibusGitlab::Util.get_env('DEPLOYER_TRIGGER_PROJECT') || '135'
  end
  ...
  def deployer_env_vars
    {
      ...
      'DEPLOY_VERSION' => OmnibusGitlab::Util.get_env('DEPLOYER_VERSION') || release,
      'DEPLOY_USER' => OmnibusGitlab::Util.get_env('DEPLOYER_DEPLOY_USER') || 'deployer'
    }
  end
end
