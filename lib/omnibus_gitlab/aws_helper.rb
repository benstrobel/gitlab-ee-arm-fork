...
class AWSHelper
  ...
  def create_ami
    release_type = OmnibusGitlab::Util.get_env('AWS_RELEASE_TYPE')
    architecture = OmnibusGitlab::Util.get_env('AWS_ARCHITECTURE')
      ...
      "DeliveryOptions" => [
        {
          "Details" => {
            "AmiDeliveryOptionDetails" => {
              "AmiSource" => {
                "AmiId" => ami_id,
                "AccessRoleArn" => OmnibusGitlab::Util.get_env('AWS_MARKETPLACE_ARN'),
                "UserName" => "ubuntu",
  ...
  def marketplace_release
    @listing_name = OmnibusGitlab::Util.get_env('AWS_LISTING_NAME')
