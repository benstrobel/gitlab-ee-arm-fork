require 'spec_helper'
require 'omnibus_gitlab/build/info/components'
...
RSpec.describe Build::Info::Components::GitLabRails do
  ...
  describe '.repo' do
    context 'when alternative sources channel selected' do
      before do
        allow(::OmnibusGitlab::Version).to receive(:sources_channel).and_return('alternative')
      end
    ...
    context 'when default sources channel' do
      before do
        allow(::OmnibusGitlab::Version).to receive(:sources_channel).and_return('remote')
      end
    ...
    context 'when security sources channel selected' do
      before do
        allow(::OmnibusGitlab::Version).to receive(:sources_channel).and_return('security')
