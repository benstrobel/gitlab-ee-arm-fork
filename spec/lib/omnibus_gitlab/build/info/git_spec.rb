require 'spec_helper'
require 'omnibus_gitlab/build/info/git'
...
RSpec.describe Build::Info::Git do
  before do
    ...
    ce_tags = "16.1.1+ce.0\n16.0.0+rc42.ce.0\n15.11.1+ce.0\n15.11.0+ce.0\n15.10.0+ce.0\n15.10.0+rc42.ce.0"
    ee_tags = "16.1.1+ee.0\n16.0.0+rc42.ee.0\n15.11.1+ee.0\n15.11.0+ee.0\n15.10.0+ee.0\n15.10.0+rc42.ee.0"
    allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with(/git -c versionsort.*ce/).and_return(ce_tags)
    allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with(/git -c versionsort.*ee/).and_return(ee_tags)
  end
    ...
    context 'in branches' do
      ...
      context 'not in CI' do
        before do
          ...
          allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with('git rev-parse --abbrev-ref HEAD').and_return('my-feature-branch')
        end
  ...
  describe '.tag_name' do
      ...
      context 'in CI' do
        ...
        context 'not in CI' do
          before do
            stub_env_var('CI_COMMIT_BRANCH', '')
            stub_env_var('CI_COMMIT_TAG', '')
            allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with('git describe --tags --exact-match').and_return('16.1.1+ee.0')
          end
    ...
    context 'if some other error is raised' do
      before do
        stub_env_var('CI_COMMIT_BRANCH', '')
        stub_env_var('CI_COMMIT_TAG', '')
        allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with('git describe --tags --exact-match').and_raise(OmnibusGitlab::Util::ShellOutExecutionError.new("", 100, "", "Some Other Error"))
      end
  ...
  describe '.commit_sha' do
    context 'from CI' do
      before do
        ...
        allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with('git rev-parse HEAD').and_return('some-other-sha')
      end
    ...
    context 'not from CI' do
      before do
        ...
        allow(OmnibusGitlab::Util).to receive(:shellout_stdout).with('git rev-parse HEAD').and_return('3cd8e712ccd3c3f356108ec1a5cbeecbf3d3be88')
      end
