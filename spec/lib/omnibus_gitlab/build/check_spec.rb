require 'spec_helper'
require 'omnibus_gitlab/build/check'
...
RSpec.describe Build::Check do
  ...
  describe 'is_auto_deploy?' do
    ...
    it 'returns false if it does not look like an auto-deploy tag' do
      ...
      allow(OmnibusGitlab::Util).to receive(:get_env).with('CI_COMMIT_REF_NAME').and_return('a-random-branch')
  ...
  describe 'ci_commit_tag?' do
    it 'checks for the CI_COMMIT_TAG' do
      allow(OmnibusGitlab::Util).to receive(:get_env).with('CI_COMMIT_TAG').and_return('11.10.12345+5159f2949cb.59c9fa631')
