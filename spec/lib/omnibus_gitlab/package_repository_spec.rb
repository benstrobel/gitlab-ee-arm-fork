require 'spec_helper'
require 'omnibus_gitlab/package_repository'
    ...
    describe "with production repository" do
      context 'with artifacts available' do
        ...
        context 'for nightly release' do
          ...
          it 'in dry run mode prints the upload commands' do
            expect { repo.upload(OmnibusGitlab::Util.get_env('STAGING_REPO'), true) }.to output(%r{Uploading...\n}).to_stdout
            expect { repo.upload(OmnibusGitlab::Util.get_env('STAGING_REPO'), true) }.to output(%r{bin/package_cloud push gitlab/nightly-builds/ubuntu/focal pkg/ubuntu-focal/gitlab.deb --url=https://packages.gitlab.com\n}).to_stdout
          end
        end
