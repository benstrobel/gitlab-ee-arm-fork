require 'spec_helper'
require 'omnibus_gitlab/version'

RSpec.describe OmnibusGitlab::Version do
  before do
  ...
  describe :remote do
    subject { OmnibusGitlab::Version.new(software) }
  ...
  describe :read_version_from_file do
    context 'when build facts exist' do
      ...
      it 'uses the version from build_facts' do
        ...
        components.each do |software|
          expect(OmnibusGitlab::Version.new(software).read_version_from_file).to eq('version-from-build-facts')
        end
      end
    end
    ...
    context 'when build facts do not exist' do
      ...
      it 'uses the version from version files' do
        ...
        components.each do |software|
          expect(OmnibusGitlab::Version.new(software).read_version_from_file).to eq('version-from-version-file')
        end
      end
    end
  end
  ...
  describe :print do
    subject { OmnibusGitlab::Version.new(software, version) }
  ...
  def mock_sources_channel(channel = 'remote')
    allow(::OmnibusGitlab::Version).to receive(:sources_channel).and_return(channel)
  end

  def mock_fallback_channel(channel = 'remote')
    allow(::OmnibusGitlab::Version).to receive(:fallback_sources_channel).and_return(channel)
  end
end
