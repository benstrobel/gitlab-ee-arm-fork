...
RSpec.describe DockerHelper do
  ...
  describe '.cleanup_existing_builder' do
    before do
      allow(Open3).to receive(:popen2e).with("docker", "buildx", "rm", any_args).and_return(true)
    end

    it 'calls docker buildx rm command with correct arguments' do
      expect(Open3).to receive(:popen2e).with(*%w[docker buildx rm --force omnibus-gitlab-builder || true])
